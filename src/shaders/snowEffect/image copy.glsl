...
void main() {
    ...
    vec3 bn = texture(iChannel0, gl_FragCoord.xy / 1024.0 + iTime * 0.2).xyz;
    vec2 uv = (gl_FragCoord.xy + (bn.xy * 2.0 - 1.0) / 3.0) / czm_viewport.zw;
    uv = uv * 2.0 - 1.0;
    ...
    vec3 dist = p - ori;
    vec3 n = getNormal(p, dot(dist, dist) * (1e-1 / czm_viewport.z));
