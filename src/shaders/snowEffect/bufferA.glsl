...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if(iFrame < 10 || texture(iChannel3, vec2(82.5 / 256., .2)).r > 0.) {
        ...
        // tiled landscape
        lh = mix(mix(getLand((fragCoord - o.xx * res) / ts), getLand((fragCoord - o.yx * res) / ts), suv.x), mix(getLand((fragCoord - o.xy * res) / ts), getLand((fragCoord - o.yy * res) / ts), suv.x), suv.y);
